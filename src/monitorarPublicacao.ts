...
interface MonitorarPublicacaoParams {
  parametro: string;
  filtro?: string;
}

async function monitorarPublicacao(params: MonitorarPublicacaoParams): Promise<void> {
  try {
    ...
    console.log(`Executando monitorarPublicacao com parametro=${params.parametro}...`);
    ...
    const resultado = {
      ...
      status: "sucesso",
      mensagem: "Configura monitoramento de publicações concluído com sucesso",
      resultado: {
...
export default monitorarPublicacao;
